...
MACROS: list[Macro] = [
    ...
    Macro("PeerRsRecords", "records", "PAF lines shmap-rs writes on the peer benchmark.",
          "paper/generated/<arch>/table_peers.tsv :: records",
          lambda c: agg(lambda v: str(int(v)), lambda v: v,
                        _peer_num(c, SUBJECT, "records")), group=True,
          caveats=("Records are PAF lines, not reads mapped. shmap-rs emits one per "
                   "mapped read so the two coincide for it; they do not for a tool "
                   "emitting secondary alignments.",)),
    Macro("PeerWinnowRecords", "records", "PAF lines Winnowmap2 writes on the same reads.",
          "paper/generated/<arch>/table_peers.tsv :: records",
          lambda c: agg(lambda v: str(int(v)), lambda v: v,
                        _peer_num(c, "winnowmap2", "records")), group=True,
          caveats=("Exceeds the read count, because Winnowmap2 emits secondary "
                   "alignments. Output volume, never sensitivity.",)),
    Macro("PeerRsIndexS", "s", "shmap-rs indexing seconds on the peer benchmark.",
          "paper/generated/<arch>/table_peers.tsv :: index_s",
          lambda c: agg(f1, lambda v: v, _peer_num(c, SUBJECT, "index_s"))),
    Macro("PeerCppIndexS", "s", "C++ indexing seconds on the same reference.",
          "paper/generated/<arch>/table_peers.tsv :: index_s",
          lambda c: agg(f1, lambda v: v, _peer_num(c, REFERENCE, "index_s"))),
