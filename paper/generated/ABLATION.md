...
| host | `galaxy` — unknown, 128 cores |
| commit | `fbff1afcf41b` **(working tree dirty)** |
| rustc | `unknown` |
| measured | 2026-08-16 |
| parameters | suite.toml `params.paper`: `-k 25 -r 0.01 -t 0.4 -d 0.075 -o 0.3`, metric `Containment` |
| thread counts | `-@1`, `-@8` |
| repeats | 5, round-robin over the whole ladder, reduced by median |
| inputs verified | bytes+records+bases against `benchmarks/data/datasets.tsv` |
| mapping digest | `c06e9dfd2761be8e` — identical on every rung |
| input digest | `sha256:0a0100703a4f65ba` |

## Inputs

| id | rel path | bytes | records | bases |
|---|---|---|---|---|
| `D6-ONT24K` | `ont24k/reads_ont24k.fa` | 2193635779 | 92220 | 2191178243 |
| `REF-HS1` | `_paper_work/hs1.fa` | 3179638084 | 25 | 3117292070 |
...
```
  C++ / shipped   = 2.64x      measured directly
  C++ / baseline  = 2.15x      the port itself: row 8 plus the rewrite
  baseline / ship = 1.23x      the whole ladder
                    2.15 x 1.23 = 2.64x
```
...
Carrying the switches at all costs 1.00x
...
at 1.01x, because the figure puts an
