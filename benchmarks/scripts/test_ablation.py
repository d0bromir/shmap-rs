...
print("the reconciliation with the C++ comparison is an identity, not a story")
# The note quotes two speedups that look contradictory -- 2-3x over the C++,
# and a ladder worth much less end to end. The defence is that they compose by
# multiplication because the ladder's baseline is the port, not the C++. That
# defence is only worth anything if the product actually returns the measured
# total, so it is checked rather than trusted.
rec = a.reconciliation()
if rec is None:
    print("  [skip] no reconciliation measured on this host (needs a C++ binary)")
else:
    r = rec["ratios"]
    check("C++/baseline x baseline/shipped returns the measured C++/shipped",
          round(r["port"] * r["ladder"], 2), round(r["total"], 2))
    check("the reconciliation is single-threaded, as the C++ is by design",
          rec["threads"], 1)
    check("and carries no profiling, which the C++ has no equivalent of",
          rec["profiling"], False)
    check("carrying the switches costs nearly nothing",
          abs(r["instrumentation_overhead"] - 1.0) < 0.10, True)
...
with tempfile.TemporaryDirectory() as td:
    ...
    check("the caption says both absences out loud",
          all(re.search(rf"row~{n}\b", files[f"{a.NAME}.tex"], re.I)
              for n in sorted(a.NOT_ABLATED)), True)
