...
with threads**, with identical mapping counts. Memory is ~7.1x lower single-threaded (2.67 GB against 18.85 GB) — but it grows with thread count, reaching 17.15 GB at the highest, so size for the run you intend.
...
|---|---:|---:|---:|---:|---:|
| HiFi, 23.2 kb, 149 438 reads | **39.9 s** | **14.4 s** | 109.3 s | **2.74x** / 7.59x | 2.65 GB vs 18.85 GB |
| ONT, 23.8 kb, 92 220 reads | **21.8 s** | **8.6 s** | 57.2 s | **2.63x** / 6.67x | 2.60 GB vs 18.85 GB |

Across all 9 benchmarks and three metrics: **2.07–2.95x** single-threaded, **6.35–7.59x** at `-@4`. Every figure here is generated from the current result set and checked in CI — see [RESULTS.md](RESULTS.md).

- **Scales to many threads** — up to **17.94x** whole-run at `-@ 32`; the C++ cannot use more than one core. Output is byte-identical at every thread count.
...
The fixed-window adaptive placement architecture has been **retired after failing
accuracy qualification**. `--adaptive` and `--adaptive-dense` now fail explicitly;
sampled placements are no longer generated by production mapping. The original
mapper remains available, together with output-preserving compact/persistent
indexing, batching, and parallel FASTA parsing. See the
[accuracy-first roadmap](docs/long_read_redesign.md#accuracy-first-direction).

The [truth-based MAPQ qualification study](benchmarks/results/adaptive-mapq-442928c/README.md)
tested a frozen offline model on independent synthetic references. Sample-preserving
mosaics exposed a fast-path failure, and repeat/noisy confidence strata lacked
support. The model is not deployed and the adaptive path is rejected, not relabeled
as accurate. The original mapper also fails some adversarial cases; retirement
does not establish Winnowmap-level accuracy. That is the long-term target.
...
passed all 540 native invocations with unchanged output. The now-retired adaptive/parser modes
